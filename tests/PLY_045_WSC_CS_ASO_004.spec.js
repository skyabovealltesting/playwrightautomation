...
test.beforeAll(async ({ browser }) => {
  ...
  test.skip('TC_01-Selecting office and Searching the job', async()=>{
    const workPackagePage = new WorkPackage(tab0, test.info());
    ...
    await workPackagePage.officeSelector(jobsData.office);
    await workPackagePage.searchAndNavigateToJob(jobsData.oppyID);
    tab1 = await commonFunction.newTab(workPackagePage.firstJobInList);
  });
  ...
  test.describe('TS_01-Creating a work package', async()=>{
    ...
    test(`TC_01-Creating a work package ${WpSiteNamesData.siteNameAddress}`, async({ })=>{
      ...
      await workPackagePage2.createControlsWorkPackage(WpSiteNamesData.workPackageName);
  ...
  test(`TC_02-Add cost items ${WpSiteNamesData.siteNameAddress}`, async ({  }) => {
    ...
    const waitFunction = new WaitFunctions(tab2);
      ...
      // Adding PLC Trane Material cost item
      await workPackagePage.addTraneMaterialInCostItem();
  ...
  test(`TC_03-Submit Work Package ${WpSiteNamesData.siteNameAddress}`, async ({ }) => {
     const wpPage2 = new WorkPackage(tab2, test.info());
      ...
      // Appending PSD for EQ-S
      await wpPage2.appendPSDtoJob(jobsData.addTM.path);
  });
  });
  });
  ...
  test('TC_01_Navigate to Bids and update purchasing Document Section ', async ({ }) => {
    const cpPage = new CreditProject(tab2, test.info());
