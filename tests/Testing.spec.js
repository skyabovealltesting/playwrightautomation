...
test.beforeAll(async ({ browser }) => {
  ...
  test('TC_04-Enable Oracle and Assigning PSD to the job', async()=>{
    ...
    await workPackagePage.appendPSDtoJob(jobsData.addTM.path2);
  });
  ...
  test.describe('TS_01-Creating a work package', async()=>{
    ...
    dataProviderForWP.forEach(MiltipleWpData =>{
    test(`TC_01-Creating a work package ${MiltipleWpData.workPackageName}`, async({ })=>{
      ...
      await workPackagePage2.createIndustrialRefrigerationWorkPackage(MiltipleWpData.workPackageName);
      await workPackagePage2.selectSiteName(MiltipleWpData.siteNameAddress);
      await workPackagePage2.updatePartsCenter(wpData.partsCenter.partCenter);
      console.log(MiltipleWpData.workPackage);
    });
  //**CREATE WORKPACKAGE AND CONFIGURE COSTITEMS*/
  test(`TC_02-Add cost items ${MiltipleWpData.workPackageName}`, async ({  }) => {
    ...
    const waitFunction = new WaitFunctions(tab2);
      ...
      let traneMaterial = String(traneMaterialName+currentCount);
      await workPackagePage.addTraneMaterial(traneMaterial);
  ...
  test(`TC_03-Submit Work Package ${MiltipleWpData.workPackageName}`, async ({ }) => {
  ...
  test('Adding Trane Material for EQ-S', async() => {
    const workPackagePage = new WorkPackage(tab1, test.info());
    await workPackagePage.appendPSDtoJob(jobsData.addTM.path);
  })
  ...
  WpProjectMngrData.forEach(projectMngrData =>{
    test(`TC_02-update Office and People section ${projectMngrData.workPackageName}`, async({ })=>{
      const cpPage = new CreditProject(tab1, test.info());
  ...
  test('TC_03_Update - ShipToAddress, JobInformation ,Billing, Classification, ComissionSplit, CreditSupplement , Literature , viewCreditProject and Transmit ', async ({ }) => {
    ...
    await cpPage.viewCreditProjectandTransmitForASO();
   // await cpPage.waitForTransmit();
   // await cpPage.afterTransmitValidation();
  });
  test('TC_03-ASO', async()=>{
    const cpPage = new CreditProject(tab1, test.info());
  ...
  //////
// test('TC_01-Selecting office and Searching the job', async()=>{
//   const cpPage = new CreditProject(tab0, test.info());
//  // const workPackagePage = new WorkPackage(tab1, test.info());
//   const commonFunction = new CommonFunctions(tab0, test.info());
//   const jobsPage = new Jobs(tab0, test.info());
//   const loginPage = new LoginPage(tab0, test.info());
//   await loginPage.loginWsWithTestaccount14(data.login.url,data.login.userName17, data.login.passWord);

//   await cpPage.searchAndNavigateToCP("F118625");//F118665//F118625
//   tab1 = await commonFunction.newTab(jobsPage.projectNumbLink);
// });
